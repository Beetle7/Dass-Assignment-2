...
Tracks player funds, positions, and properties.
"""
...
class Player:
    ...
    def move(self, steps):
        ...
        """
        new_pos = (self.position + steps) % BOARD_SIZE
        passed_go = (self.position + steps) >= BOARD_SIZE
        self.position = new_pos

        if passed_go or self.position == 0:
            self.add_money(GO_SALARY)
            print(f"  {self.name} passed/landed on Go and collected ${GO_SALARY}.")
